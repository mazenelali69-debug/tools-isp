...
function HeatBar({ value, maxValue = 320 }) {
  const blocks = 64;
  const safe = Math.max(0, num(value));
  const active = Math.max(0, Math.min(blocks, Math.round((safe / maxValue) * blocks)));
        ...
        return (
          <span
            ...
            className={
              "dashx-heat__cell " +
              (on ? "is-on " : "is-off ") +
              (i < 18 ? "is-good" : i < 38 ? "is-warn" : i < 54 ? "is-high" : "is-critical")
            }
          />
        );
      })}
      ...
      {rows.map((row, idx) => {
        ...
        return (
          <div key={row.id} className="dashx-rankRow dashx-rankRow--true">
            <div className="dashx-rankRow__head">
              <div className="dashx-rankRow__left">
                <span className={"dashx-rankDot is-" + tone} />
                <span className="dashx-rankIndex">#{idx + 1}</span>
                <span className="dashx-rankRow__name">{row.name || row.id}</span>
              </div>

              <div className="dashx-rankRow__value">{Math.round(num(row.totalMbps) * 100) / 100} Mbps</div>
            </div>
