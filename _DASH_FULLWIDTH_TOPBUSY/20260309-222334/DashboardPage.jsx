...
function PressureList({ rows }) {
  ...
  return (
    <div className="gbg-list">
      {rows.map((row, idx) => {
        const tone = getPressureStatus(row.totalMbps);

        return (
          <div key={row.id} className="gbg-row">
            <div className="gbg-row__head">
              <div className="gbg-row__left">
                <span className={"gbg-dot is-" + tone} />
                <span className="gbg-rank">#{idx + 1}</span>
                <span className="gbg-name">{row.name || row.id}</span>
              </div>

              <div className="gbg-row__right">
                <span className="gbg-value">{fmtMbps(row.totalMbps)}</span>
              </div>
            </div>

            <div className="gbg-row__bar">
              <StripMeter value={row.totalMbps} maxValue={maxValue} />
            </div>
          </div>
        );
      })}
...
export default function DashboardPage() {
  ...
  return (
    <div className="dashx-page">
      ...
      <section className="dashx-row dashx-row--strip dashx-row--grafana">
