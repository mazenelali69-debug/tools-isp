...
function HeatBar({ value, maxValue = 220 }) {
  ...
  const active = Math.max(1, Math.round(blocks * ratio));

  return (
    <div className="dashx-heat dashx-heat--true">
      {Array.from({ length: blocks }, (_, i) => {
        ...
        if (i < active) {
          ...
          else if (p < 0.62) cls += " is-lime";
          else if (p < 0.82) cls += " is-amber";
          else cls += " is-red";
        } else {
          cls += " is-off";
        }
...
function PressureList({ rows }) {
  ...
  return (
    <div className="dashx-rankList dashx-rankList--true">
      {rows.map((row, idx) => {
        const tone = getPressureStatus(row.totalMbps);

        return (
          <div key={row.id} className="dashx-rankRow dashx-rankRow--true">
            <div className="dashx-rankRow__headTrue">
              <div className="dashx-rankRow__leftTrue">
                <span className={"dashx-rankDot dashx-rankDot--true is-" + tone} />
                <span className="dashx-rankIndex dashx-rankIndex--true">#{idx + 1}</span>
                <span className="dashx-rankRow__name dashx-rankRow__name--true">{row.name || row.id}</span>
              </div>

              <div className="dashx-rankRow__rightTrue">
                <span className="dashx-rankRow__value dashx-rankRow__value--true">{fmtMbps(row.totalMbps)}</span>
              </div>
            </div>

            <div className="dashx-rankRow__barWrap dashx-rankRow__barWrap--true">
              <HeatBar value={row.totalMbps} maxValue={maxValue} />
            </div>
          </div>
        );
      })}
...
export default function DashboardPage() {
  ...
  return (
    <div className="dashx-page">
      ...
      <section className="dashx-row dashx-row--topBusyTuned">
