...
function DeviceRow({ item, zebra, trafficHistory }) {
  ...
  const rxTone = isAviat ? "#00e5ff" : "#22d3ee";
  const txTone = isAviat ? "#ffb703" : "#60a5fa";
        ...
        {item.ifIndex ? (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <TrafficSpark
              series={rxSeries}
              color={rxTone}
              label={(item.ip === "155.15.59.4" ? "AVIAT RX · ifIndex " : "RX Mbps · ifIndex ") + item.ifIndex}
              valueText={fmtMbps(currentRx)}
            />
            <TrafficSpark
              series={txSeries}
              color={txTone}
              label={(item.ip === "155.15.59.4" ? "AVIAT TX · ifIndex " : "TX Mbps · ifIndex ") + item.ifIndex}
