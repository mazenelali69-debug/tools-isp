...
function HeatBar({ value }) {
  ...
  const active = Math.max(1, Math.min(blocks, Math.round((safe / max) * blocks)));
  ...
  return (
    <div className="dashx-heat">
      {Array.from({ length: blocks }, (_, i) => {
        ...
        if (i < active) {
          const p = i / blocks;
          if (p < 0.30) cls += " is-good";
          else if (p < 0.58) cls += " is-warn";
          else if (p < 0.80) cls += " is-high";
          else cls += " is-critical";
        } else {
          cls += " is-off";
        }
...
function PressureList({ rows }) {
  ...
  return (
    <div className="dashx-rankList">
      {rows.map((row, idx) => {
        const pct = Math.max(4, Math.min(100, (num(row.totalMbps) / max) * 100));
        const tone = getPressureStatus(row.totalMbps);

        return (
          <div key={row.id} className="dashx-rankRow">
            <div className="dashx-rankRow__top">
              <div className="dashx-rankRow__left">
              ...
              <div className="dashx-rankRow__right">
                <div className={"dashx-miniBadge is-" + tone}>{statusLabel(tone)}</div>
                <div className="dashx-rankRow__value">{fmtMbps(row.totalMbps)}</div>
              </div>
            </div>

            <div className="dashx-rankRow__barWrap">
...
export default function DashboardPage() {
  ...
  return (
    <div className="dashx-page">
      ...
      <section className="dashx-row dashx-row--bottom">
        <div className="dashx-panel">
          <TrendChart items={history} />
        </div>

        <div className="dashx-panel">
          <div className="dashx-blockTitle">Quick context</div>
          <div className="dashx-blockSub">Stable summary of current state</div>

          <div className="dashx-kv">
            <div className="dashx-kv__row"><span>Latest history rows</span><strong>{history.length}</strong></div>
            <div className="dashx-kv__row"><span>Strongest uplink</span><strong>{latest?.name || "-"}</strong></div>
            <div className="dashx-kv__row"><span>Top pressure now</span><strong>{latest ? fmtMbps(latest.totalMbps) : "-"}</strong></div>
            <div className="dashx-kv__row"><span>Status tier</span><strong>{statusLabel(pressure)}</strong></div>
          </div>
        </div>
      </section>
    </div>
